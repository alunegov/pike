...
void RemoteServerImpl::Start()
{
    ...
    _auto_reset_thread = std::thread{[this]() {
        // «адержка между проверками дл€ авто-сброса движени€
...
{
    // √раница детектировани€ движени€, [0-1]
    ...
    // ѕо услови€м ниже выходит, что сначала нужно остановить вращение и только потом начинать движение - нужны два
    // пакета. Ќо дл€ перехода из движени€ во вращение достаточно одного - _move_state сбрасываетс€ по коду раньше.

    if (_move_state != new_move_state) {
        if (_rot_state == MotionDirection::No) {  // нет вращени€
    ...
    if (_rot_state != new_rot_state) {
        if (_move_state == MotionDirection::No) {  // нет движени€
...
{
    // ѕериод "неприхода" данных от клиента, после которого будет авто-сброс движени€
